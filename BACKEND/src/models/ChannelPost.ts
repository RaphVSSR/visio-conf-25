		...
		try {
			for (const channel of await Channel.model.find({})) {
				for (const channelPost of [
					...
					if (channel.members && channel.members.length > 0) {
						for (const member of await Promise.all(
							channel.members.map(async (memberId) => await ChannelMember.model.findById(memberId)),
						)) {
				...
				if (process.env.VERBOSE === "true" && process.env.VERBOSE_LVL === "3") {
					console.log(
						`ðŸ’¾ ${await ChannelPost.model.countDocuments({ channelId: channel._id })} posts created for "${channel.name}"`,
					);
				}
			}
