...
export default class ChatService {
	...
	private handleChatOperation = async (socketId: string, payload: { action: string; data: any }) => {
		...
		if (!userId)
			return this.send(socketId, "chat_operation_result", {
		...
		try {
			switch (payload.action) {
				case "CREATE": {
					...
					if (type === "unique") {
						if (uniqueMembers.length !== 2) {
							return this.send(socketId, "chat_operation_result", {
								action: "CREATE",
								status: "error",
								message: "invalid_unique_members",
							});
						}
						const existing = await Discussion.model.findOne({
							type: "unique",
					...
					this.send(targets.length > 0 ? targets : socketId, "chat_operation_result", {
						action: "CREATE",
						status: "success",
						data: populatedChat,
					});
	...
	private handleMessageOperation = async (socketId: string, payload: { action: string; data: any }) => {
		const userId = SessionManager.getUserId(socketId);
		if (!userId)
			return this.send(socketId, "message_operation_result", {
				action: payload.action,
				status: "error",
				message: "not_authenticated",
			});
		...
		try {
			switch (payload.action) {
				case "SEND": {
					...
					if (
						...
						payload.data.content.length > ChatService.MAX_CONTENT_LENGTH
					) {
						return this.send(socketId, "message_operation_result", {
							action: "SEND",
							status: "error",
					...
					await Discussion.model.updateOne({ _id: discussion._id }, { $push: { messages: newMessage } });

					const targets = await this.getMemberSocketIds(discussion.members);
					this.send(targets.length > 0 ? targets : socketId, "message_operation_result", {
						action: "SEND",
						status: "success",
						data: { chatUuid: discussion.uuid, message: newMessage },
					});
					break;
				}
				...
				case "DELETE": {
					...
					this.send(targets.length > 0 ? targets : socketId, "message_operation_result", {
						action: "DELETE",
						status: "success",
						data: { chatUuid: discussion.uuid, messageUuid: payload.data.messageUuid },
					});
					break;
				}
			}
			...
			this.send(socketId, "message_operation_result", {
				action: payload.action,
				status: "error",
				message: err.message,
			});
