...
export default class ChannelService {
	...
	private createChannel = async (
		socketId: string,
		payload: { name: string; isPublic: boolean; teamId: string; members?: string[] },
	) => {
		const userId = this.resolveUserId(socketId);
		if (!userId)
			return this.send(socketId, "channel_action_response", {
				type: "create",
				etat: false,
		...
		if (isPublic) {
			...
			for (const teamMember of teamMembers) {
				...
				const channelMember = new ChannelMember({
					...
					userId: teamMemberUserId as any,
					role: "member",
				});
	...
	private updateChannel = async (
		socketId: string,
		payload: { channelId: string; name: string; isPublic: boolean; teamId: string; members?: string[] },
	) => {
		const userId = this.resolveUserId(socketId);
		if (!userId)
			return this.send(socketId, "channel_action_response", {
	...
	private getChannelMembers = async (socketId: string, payload: { channelId: string }) => {
		...
		const members = await ChannelMember.model
			.find({ channelId })
	...
	private removeChannelMember = async (socketId: string, payload: { channelId: string; userId: string }) => {
		const requesterId = this.resolveUserId(socketId);
		if (!requesterId)
			return this.send(socketId, "channel_member_response", {
				type: "remove",
				etat: false,
				error: "not_authenticated",
			});
	...
	private getChannelPosts = async (socketId: string, payload: { channelId: string }) => {
		...
		const posts = await ChannelPost.model
			.find({ channelId })
		...
		const formattedPosts = await Promise.all(
			posts.map(async (post: any) => {
				const responses = await ChannelPostResponse.model
	...
	private getUserPost = async (socketId: string, payload: { channelId: string; userId: string }) => {
		...
		const posts = await ChannelPost.model
			.find({ channelId, authorId: targetUserId })
			.populate("authorId", "firstname lastname email picture")
			.sort({ createdAt: 1 })
			.lean();
	...
	private publishPost = async (socketId: string, payload: { channelId: string; content: string }) => {
		...
		const populatedPost = await ChannelPost.model
			.findById(newPost.modelInstance._id)
			.populate("authorId", "firstname lastname email picture")
			.lean();
	...
	private answerPost = async (socketId: string, payload: { postId: string; content: string }) => {
		...
		if (post.authorId.toString() === userId)
			return this.send(socketId, "channel_post_response", {
				type: "answer",
				etat: false,
				error: "cannot_answer_own_post",
			});
		...
		const populatedResponse = await ChannelPostResponse.model
			.findById(newResponse.modelInstance._id)
			.populate("authorId", "firstname lastname email picture")
			.lean();
