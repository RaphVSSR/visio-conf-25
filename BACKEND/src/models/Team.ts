			...
			const teamsToInject = [
				{
					...
					description: "Ã‰quipe des enseignants et personnels du dÃ©partement MMI",
			...
			for (const team of teamsToInject) {
				if (!(await this.model.findOne({ name: team.name }))) {
					...
					if (process.env.VERBOSE === "true" && process.env.VERBOSE_LVL === "3")
						console.log(`ðŸ’¾ New team "${team.name}" created`);
				} else {
					if (process.env.VERBOSE === "true" && process.env.VERBOSE_LVL === "3")
						console.log(`ðŸ’¾ Team "${team.name}" already exists`);
				}
			}
