...
export default class Team extends Collection {
    ...
    static model = models.Team || model("Team", this.schema);
    modelInstance;
            ...
            const teamsToInject = [
                {
                    ...
                    description: "Équipe des enseignants et personnels du département MMI",
            ...
            for (const team of teamsToInject) {
                if (!await this.model.findOne({ name: team.name })) {
                    ...
                    if (process.env.VERBOSE === "true" && process.env.VERBOSE_LVL === "3")
                        console.log(`💾 New team "${team.name}" created`);
                }
                else {
                    if (process.env.VERBOSE === "true" && process.env.VERBOSE_LVL === "3")
