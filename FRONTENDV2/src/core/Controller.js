export class Controleur {
  ...
  inscription(emetteur, liste_emission, liste_abonnement) {
    for (var key in liste_emission) {
      ...
      if (
        typeof this.listeEmission[liste_emission[key]][emetteur.nomDInstance] !=
        "undefined"
      ) {
        console.log(
          "ERREUR(controleur: " +
    ...
    for (var key in liste_abonnement) {
      ...
      if (
        ...
        this.listeAbonnement[liste_abonnement[key]][emetteur.nomDInstance] =
          emetteur;
      }
    }
  }
  ...
  envoie(emetteur, t) {
    ...
    for (var item in t) {
      if (item != "id") {
        ...
        if (
          typeof this.listeEmission[item][emetteur.nomDInstance] == "undefined"
        ) {
          console.log(
            "ERREUR (controleur): Le message " +
              item +
        ...
        for (var recepteurkey in this.listeAbonnement[item]) {
          ...
          if (this.verboseall || this.verbose) {
            console.log(
              "INFO (controleur): on envoie " + item + " à " + recepteurkey,
            );
          }
